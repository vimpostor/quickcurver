...
In general prefer to use the script `./build.sh`. However, if the script fails for an unexplainable reason, you may want to try to execute the following commands manually instead of `./build.sh`:
```{r, engine='bash', count_lines}
